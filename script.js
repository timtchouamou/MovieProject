...
async function searchOMDb() {
      ...
      if (!term) {
        ...
        return;
                  // Redirect to index2.html with ?q=term
      window.location.href = `index2.html?q=${encodeURIComponent(term)}`;
...
let img = document.getElementById("movingImage");
    let position = 0; // starting point in pixels
    let speed = 2;    // pixels per frame

    function moveImage() {
        position -= speed;
        
        // If the image moves off screen, reset to start
        if (position > window.innerWidth) {
            position = -img.width; 
        }

        img.style.left = position + "px";
        requestAnimationFrame(moveImage); // smooth animation
    }
